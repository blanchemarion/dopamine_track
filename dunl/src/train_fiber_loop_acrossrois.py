...
def init_params():
    ...
    parser.add_argument(
        ...
        type=str,
        help="config folder",
        default=r"dunl-compneuro\config",
    )
    parser.add_argument(
        "--config-filename",
        type=str,
        help="config filename",
        default="photometry_dopamine_trail_expert_config.yaml",#"photometry_dopamine_trail_config.yaml",
    )
...
def main(params, random_date):
    ...
    for data_path_cur in data_path_list:
        ...
        out_path = os.path.join(
            "dopamine",
            "results",
            "{}_{}_numwindow{}_neuron{}_kernellength{}_{}kernels_{}unroll_{}".format(
                params["exp_name"],
                re.search(r"general_format_processed_(.*?)_kernellength", data_path_cur).group(1),
